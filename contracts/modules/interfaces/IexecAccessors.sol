...
interface IexecAccessors
{
	...
	function viewAccount(address) external view returns (IexecLibCore_v4.Account memory);
	function token() external view returns (address);
	function viewDeal(bytes32) external view returns (IexecLibCore_v4.Deal memory);
	function viewConsumed(bytes32) external view returns (uint256);
	function viewPresigned(bytes32) external view returns (address);
	function viewTask(bytes32) external view returns (IexecLibCore_v4.Task memory);
	function viewContribution(bytes32,address) external view returns (IexecLibCore_v4.Contribution memory);
	function viewScore(address) external view returns (uint256);
	function resultFor(bytes32) external view returns (bytes memory);
	function viewCategory(uint256) external view returns (IexecLibCore_v4.Category memory);
