...
interface IexecAccessorsABILegacy {
    ...
    function viewDealABILegacy_pt2(
        bytes32 _id
    ) external view returns (uint256, bytes32, address, address, address, string memory);
    ...
    function viewContributionABILegacy(
        bytes32 _taskid,
        address _worker
    ) external view returns (IexecLibCore_v5.ContributionStatusEnum, bytes32, bytes32, address);
