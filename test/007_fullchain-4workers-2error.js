...
contract('IexecHub', async (accounts) => {
	...
	before("configure", async () => {
		...
		trusttarget = 10; // [6;22]
		workers = [
			{ address: worker1, enclave: constants.NULL.ADDRESS, raw: "je ne veux pas travailler" },
			{ address: worker2, enclave: constants.NULL.ADDRESS, raw: "<timeout reached>" },
	...
	it("[Finalized] Check balances", async () => {
		...
		balance = await IexecClerkInstance.viewAccount(worker1        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [  982, 10 ], "check balance");
		...
		balance = await IexecClerkInstance.viewAccount(worker3        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1009, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccount(worker4        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1009, 10 ], "check balance");
