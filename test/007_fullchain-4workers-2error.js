...
contract('IexecHub', async (accounts) => {
	...
	it("[Finalized] Check workorder", async () => {
		...
		assert.equal    (workorder.revealCounter.toNumber(),     workers.length - 2,                      "check workorder (workorder.revealCounter)"    );
		assert.equal    (workorder.winnerCounter.toNumber(),     workers.length - 2,                      "check workorder (workorder.winnerCounter)"    );
	...
	it("[Finalized] Check balances", async () => {
		...
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker1  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 1009, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker2  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 1009, 10 ], "check balance");
		...
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker4  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [  982, 10 ], "check balance");
