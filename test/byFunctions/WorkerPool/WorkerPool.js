...
contract('IexecHub', function(accounts) {
  ...
  it("WorkerPool_01 : test initial state after constructor execution of WorkerPool contract", async function() {
    ...
    let m_owner = await aWorkerPoolInstance.m_owner.call();
    assert.strictEqual( m_owner, scheduleProvider,"check m_owner");

    let m_description = await aWorkerPoolInstance.m_description.call();
    assert.strictEqual(m_description,"myWorkerPool 1", "check m_description");

    let m_stakeRatioPolicy = await aWorkerPoolInstance.m_stakeRatioPolicy.call();
    assert.strictEqual(m_stakeRatioPolicy.toNumber(),30 , "check m_stakeRatioPolicy");
    ...
    let m_workerPoolStatus = await aWorkerPoolInstance.m_workerPoolStatus.call();
    assert.strictEqual(m_workerPoolStatus.toNumber(),constants.WorkerPoolStatusEnum.OPEN, "check m_workerPoolStatus");
