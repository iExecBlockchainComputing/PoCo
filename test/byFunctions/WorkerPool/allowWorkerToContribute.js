...
contract('IexecHub', function(accounts) {
	...
	it("allowWorkerToContribute_01: scheduler can allowWorkerToContribute on worker who has subscribed to his workerpool", async function() {
    ...
    assert.strictEqual(weight.toNumber(), 0, "check weight");
  	...
  	it("allowWorkerToContribute_02: scheduler can't allowWorkerToContribute on a non existing workorder", async function() {
      ...
      await Extensions.expectedExceptionPromise(() => {
          return aWorkerPoolInstance.allowWorkerToContribute(aWorkerPoolInstance.address/*wrong workorder id*/,resourceProvider,0, {
            from: scheduleProvider,
            gas:constants.AMOUNT_GAS_PROVIDED
          });
        },
    ...
    it("allowWorkerToContribute_03: scheduler can't allowWorkerToContribute on worker not in his workerpool", async function() {

      //  WORKER SUBSCRIBE DO NOT TO POOL in this test
  //    txMined = await aWorkerPoolInstance.subscribeToPool({
  //      from: resourceProvider,
  //      gas: constants.AMOUNT_GAS_PROVIDED
  //    });
    ...
    it("allowWorkerToContribute_04: scheduler can allowWorkerToContribute on worker who has subscribed to his workerpool with enclave challenge set", async function() {
      ...
      //here accounts[8] is the address challenge
      txMined = await aWorkerPoolInstance.allowWorkerToContribute(woid,resourceProvider,accounts[8], {
        from: scheduleProvider
      });
      events = await Extensions.getEventsPromise(aWorkerPoolInstance.AllowWorkerToContribute({}),1,constants.EVENT_WAIT_TIMEOUT);
      assert.strictEqual(events[0].args.woid, woid, "check woid");
      ...
      assert.strictEqual(enclaveChallenge, accounts[8], "check enclaveChallenge is valorized");
      assert.strictEqual(score.toNumber(), 0, "check score");
      assert.strictEqual(weight.toNumber(), 0, "check weight");
