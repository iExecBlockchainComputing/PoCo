...
contract('IexecHub', function(accounts) {
  ...
  it("evictWorker_01 : scheduler can evict worker present in his workerpool", async function() {
    ...
    txMined = await aWorkerPoolInstance.evictWorker(resourceProvider,{
      from: scheduleProvider,
      gas: constants.AMOUNT_GAS_PROVIDED
    });
    assert.isBelow(txMined.receipt.gasUsed, constants.AMOUNT_GAS_PROVIDED, "should not use all gas");

    events = await Extensions.getEventsPromise(aIexecHubInstance.WorkerPoolEviction({}), 1, constants.EVENT_WAIT_TIMEOUT);
    ...
    events = await Extensions.getEventsPromise(aWorkerPoolInstance.WorkerEviction({}), 1, constants.EVENT_WAIT_TIMEOUT);
    assert.strictEqual(events[0].args.worker, resourceProvider, "check worker");
  ...
  it("evictWorker_02 : scheduler can't evict worker not present in his workerpool", async function() {
    txMined = await aIexecHubInstance.deposit(subscriptionLockStakePolicy + subscriptionMinimumStakePolicy, {
      from: resourceProvider,
      gas: constants.AMOUNT_GAS_PROVIDED
    });
    ...
    await Extensions.expectedExceptionPromise(() => {
        return aWorkerPoolInstance.evictWorker(iExecCloudUser,{
          from: scheduleProvider,
          gas: constants.AMOUNT_GAS_PROVIDED
        });
      },
  ...
  it("evictWorker_03 : others than scheduler can't evict worker present in the workerpool", async function() {
    txMined = await aIexecHubInstance.deposit(subscriptionLockStakePolicy + subscriptionMinimumStakePolicy, {
      from: resourceProvider,
      gas: constants.AMOUNT_GAS_PROVIDED
    });
    ...
    await Extensions.expectedExceptionPromise(() => {
        return aWorkerPoolInstance.evictWorker(resourceProvider,{
          from: resourceProvider,
          gas: constants.AMOUNT_GAS_PROVIDED
        });
      },
