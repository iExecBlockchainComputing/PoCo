...
contract('IexecHub', function(accounts) {
	...
	it("withdraw_01: can withdraw the stake amount owned by iExecCloudUser", async function() {

		balance =await aRLCInstance.balanceOf(iExecCloudUser);
		assert.strictEqual(balance.toNumber(), 1000, "check balanceOf iExecCloudUser");
		...
		assert.strictEqual(events[0].args.owner, iExecCloudUser, "check Deposit owner");
		...
		balance =await aRLCInstance.balanceOf(iExecCloudUser);
		assert.strictEqual(balance.toNumber(), 900, "check balanceOf iExecCloudUser");

		txMined = await aIexecHubInstance.withdraw(75, {
			from: iExecCloudUser,
			gas: constants.AMOUNT_GAS_PROVIDED
		});

		events = await Extensions.getEventsPromise(aIexecHubInstance.Withdraw({}));
		assert.strictEqual(events[0].args.owner, iExecCloudUser, "check Withdraw owner");
		assert.strictEqual(events[0].args.amount.toNumber(), 75, "check Withdraw amount");

		balance =await aRLCInstance.balanceOf(iExecCloudUser);
		assert.strictEqual(balance.toNumber(), 975, "check balanceOf iExecCloudUser");

		checkBalance = await aIexecHubInstance.checkBalance.call(iExecCloudUser);
		assert.strictEqual(checkBalance[0].toNumber(), 25, "check stake of the iExecCloudUser");
		assert.strictEqual(checkBalance[1].toNumber(), 0, "check stake locked of iExecCloudUser");
