...
contract('IexecHub', function(accounts) {
  ...
  it("createCategory_01 : it should be possible for the CategoriesCreator to create a new categories from the config/categories.json file", async function() {
    ...
    events = await Extensions.getEventsPromise(aIexecHubInstance.CreateCategory({}, {
      fromBlock: 0,
      toBlock: 'latest'
    }), categoriesConfigFileJson.categories.length, constants.EVENT_WAIT_TIMEOUT);

    for (var i = 0; i < events.length; i++) {
      assert.strictEqual(events[i].args.catid.toNumber(), i + 1, "catid check");
      assert.strictEqual(events[i].args.name, categoriesConfigFileJson.categories[i].name, "name check");
      assert.strictEqual(events[i].args.description, JSON.stringify(categoriesConfigFileJson.categories[i].description), "description check");
      assert.strictEqual(events[i].args.workClockTimeRef.toNumber(), categoriesConfigFileJson.categories[i].workClockTimeRef, "workClockTimeRef check");
    }

  });


  it("createCategory_02 : it should NOT be possible for others users than the CategoriesCreator to create a new categories from the config/categories.json file", async function() {
    var categoriesConfigFile = await readFileAsync("./config/categories.json");
    var categoriesConfigFileJson = JSON.parse(categoriesConfigFile);
    for (var i = 0; i < categoriesConfigFileJson.categories.length; i++) {
      await Extensions.expectedExceptionPromise(() => {
          return aIexecHubInstance.createCategory(categoriesConfigFileJson.categories[i].name, JSON.stringify(categoriesConfigFileJson.categories[i].description), categoriesConfigFileJson.categories[i].workClockTimeRef, {
            from: iExecCloudUser,
            gas: constants.AMOUNT_GAS_PROVIDED
          });
        },
