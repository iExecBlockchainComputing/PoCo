...
contract('IexecHub', function(accounts) {
  ...
  it("getCategory_01 : it should be possible for every one to get getCategory of an existing category", async function() {
    var categoriesConfigFile = await readFileAsync("./config/categories.json");
    var categoriesConfigFileJson = JSON.parse(categoriesConfigFile);

    [catid, name, description, workClockTimeRef] = await aIexecHubInstance.getCategory.call(1,{
      from: iExecCloudUser,
      gas:constants.AMOUNT_GAS_PROVIDED
    });
    assert.strictEqual(catid.toNumber(), 1, "check getCategory catid on existing category");
    assert.strictEqual(name, categoriesConfigFileJson.categories[0].name, "check getCategory name on existing category");
    assert.strictEqual(description,  JSON.stringify(categoriesConfigFileJson.categories[0].description), "check getCategory description on existing category");
    assert.strictEqual(workClockTimeRef.toNumber(), categoriesConfigFileJson.categories[0].workClockTimeRef, "check getCategory workClockTimeRef on existing category");

  });

  it("getCategory_02 : it should throw when calling getCategory of an not existing category", async function() {
    var categoriesConfigFile = await readFileAsync("./config/categories.json");
    var categoriesConfigFileJson = JSON.parse(categoriesConfigFile);
    await Extensions.expectedExceptionPromise(() => {
        return aIexecHubInstance.getCategory.call(100,{
          from: iExecCloudUser,
          gas:constants.AMOUNT_GAS_PROVIDED
        });
      },
