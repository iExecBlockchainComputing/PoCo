...
contract('IexecHub', function(accounts) {
  ...
  it("existingCategory_01 : it should be possible for every one to check existing category", async function() {
    var categoriesConfigFile = await readFileAsync("./config/categories.json");
    var categoriesConfigFileJson = JSON.parse(categoriesConfigFile);

    existingCategory = await aIexecHubInstance.existingCategory.call(1,{
      from: iExecCloudUser
    });
    assert.strictEqual(existingCategory, true, "check existing category");

  });

  it("existingCategory_02 : it should be possible for every one to check non existing category", async function() {
    var categoriesConfigFile = await readFileAsync("./config/categories.json");
    var categoriesConfigFileJson = JSON.parse(categoriesConfigFile);
    existingCategory = await aIexecHubInstance.existingCategory.call(100,{
      from: iExecCloudUser
    });
    assert.strictEqual(existingCategory, false, "check non existing category");
  });
