...
contract('IexecHub', async (accounts) => {
	...
	it("[6.2] Finalize - Error (partial - soon)", async () => {
		await odbtools.reverts(() => IexecHubInstance.finalize(tasks[2], web3.utils.utf8ToHex("aResult 2"), { from: scheduler, gas: constants.AMOUNT_GAS_PROVIDED }));
	});
	...
	it("[6.4] Finalize - Error (no contribution)", async () => {
		await odbtools.reverts(() => IexecHubInstance.finalize(tasks[4], web3.utils.utf8ToHex("aResult 4"), { from: scheduler, gas: constants.AMOUNT_GAS_PROVIDED }));
	});

	it("[6.5] Finalize - Error (no consensus)", async () => {
		await odbtools.reverts(() => IexecHubInstance.finalize(tasks[5], web3.utils.utf8ToHex("aResult 5"), { from: scheduler, gas: constants.AMOUNT_GAS_PROVIDED }));
	});

	it("[6.6] Finalize - Error (no reveal)", async () => {
		await odbtools.reverts(() => IexecHubInstance.finalize(tasks[6], web3.utils.utf8ToHex("aResult 6"), { from: scheduler, gas: constants.AMOUNT_GAS_PROVIDED }));
	});
	...
	it("[6.7] Finalize - Error (late)", async () => {
		await odbtools.reverts(() => IexecHubInstance.finalize(tasks[7], web3.utils.utf8ToHex("aResult 7"), { from: scheduler, gas: constants.AMOUNT_GAS_PROVIDED }));
	});
});
