...
contract('IexecHub', async (accounts) => {
	...
	it("[setup] Initialization", async () => {
		tasks[1] = extractEvents(await IexecHubInstance.initialize(deals[1], 1, { from: scheduler }), IexecHubInstance.address, "TaskInitialize")[0].args.taskid; // full
		tasks[2] = extractEvents(await IexecHubInstance.initialize(deals[1], 2, { from: scheduler }), IexecHubInstance.address, "TaskInitialize")[0].args.taskid; // partial to soon
		tasks[3] = extractEvents(await IexecHubInstance.initialize(deals[1], 3, { from: scheduler }), IexecHubInstance.address, "TaskInitialize")[0].args.taskid; // partial ok
		tasks[4] = web3.utils.soliditySha3({ t: 'bytes32', v: deals[1] }, { t: 'uint256', v: 4 });                                                                // uninitialized
		tasks[5] = extractEvents(await IexecHubInstance.initialize(deals[1], 5, { from: scheduler }), IexecHubInstance.address, "TaskInitialize")[0].args.taskid; // no consensus
		tasks[6] = extractEvents(await IexecHubInstance.initialize(deals[1], 6, { from: scheduler }), IexecHubInstance.address, "TaskInitialize")[0].args.taskid; // no reveal
		tasks[7] = extractEvents(await IexecHubInstance.initialize(deals[1], 7, { from: scheduler }), IexecHubInstance.address, "TaskInitialize")[0].args.taskid; // late
	});
	...
	it("[setup] Reveal", async () => {
		...
		await IexecHubInstance.reveal(tasks[1], odbtools.hashResult(tasks[1], "true").digest, { from: worker2 });
