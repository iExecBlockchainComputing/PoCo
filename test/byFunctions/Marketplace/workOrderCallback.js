...
contract('IexecHub', function(accounts) {
  ...
  it("workOrderCallback_01: test workOrderCallback from a smart contract", async function() {
    ...
    await aRLCInstance.transfer(aIexecAPIInstance.address, 100, {
      from: iExecCloudUser,
      gas: constants.AMOUNT_GAS_PROVIDED
    });
    ...
    assert.strictEqual(events[0].args.callbackTo, aIexecAPIInstance.address, "check  callbackTo = aIexecAPIInstance");
    assert.strictEqual(events[0].args.gasCallbackProvider, scheduleProvider, "check  gasCallbackProvider = scheduleProvider pay it");
  ...
  it("workOrderCallback_02: test every body can callback", async function() {
    ...
    assert.strictEqual(events[0].args.beneficiary, marketplaceCreator, "check  beneficiary = marketplaceCreator in our case");
    assert.strictEqual(events[0].args.callbackTo, aIexecAPIInstance.address, "check  callbackTo = aIexecAPIInstance");
    assert.strictEqual(events[0].args.gasCallbackProvider, resourceProvider, "check  gasCallbackProvider = resourceProvider pay it");
    assert.strictEqual(events[0].args.stdout, "aStdout", "check m_stdout");
    assert.strictEqual(events[0].args.stderr, "aStderr", "check m_stderr");
    assert.strictEqual(events[0].args.uri, "anUri", "check m_uri");
