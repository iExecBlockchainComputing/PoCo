...
contract('IexecHub', async (accounts) => {
	...
	it("[setup] Initialization", async () => {
		...
		tasks[8] = extractEvents(await IexecHubInstance.initialize(deals[1], 8, { from: poolScheduler }), IexecHubInstance.address, "ConsensusInitialize")[0].args.taskid;
		tasks[9] = extractEvents(await IexecHubInstance.initialize(deals[1], 9, { from: poolScheduler }), IexecHubInstance.address, "ConsensusInitialize")[0].args.taskid;
	});
	...
	it("[setup] Consensus", async () => {
		...
		await IexecHubInstance.revealConsensus(tasks[8], odbtools.hashResult("true").contribution.hash, { from: poolScheduler });
		// await IexecHubInstance.revealConsensus(tasks[9], odbtools.hashResult("true").contribution.hash, { from: poolScheduler });
	});
	...
	it("[4.8] Reveal - Error (late for reveal)", async () => {
		try {
			await IexecHubInstance.reveal(
				tasks[8],
				odbtools.hashResult("true").base,
