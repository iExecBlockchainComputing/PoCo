...
contract('IexecHub', function(accounts) {
  ...
  it("createDataset_01 : the owner of ( here marketplaceCreator ) can call createDataset and create several Dataset", async function() {
    ...
    isDatasetRegistred = await aDatasetHubInstance.isDatasetRegistred.call(createdDataset);
    assert.isFalse(isDatasetRegistred, "check isDatasetRegistred false");
    ...
    getDataset = await aDatasetHubInstance.getDataset.call(marketplaceCreator, 1);
    assert.notEqual(getDataset, createdDataset, "check getDataset");
    ...
    isDatasetRegistred = await aDatasetHubInstance.isDatasetRegistred.call(createdDataset);
    assert.isTrue(isDatasetRegistred, "check isDatasetRegistred");

    getDatasetsCount = await aDatasetHubInstance.getDatasetsCount.call(marketplaceCreator);
    assert.strictEqual(getDatasetsCount.toNumber(), 1, "check getDatasetsCount");

    getDataset = await aDatasetHubInstance.getDataset.call(marketplaceCreator, 1);
    assert.strictEqual(getDataset, createdDataset, "check getDataset");
    ...
    isDatasetRegistred = await aDatasetHubInstance.isDatasetRegistred.call(createdDataset2);
    assert.isTrue(isDatasetRegistred, "check isDatasetRegistred");

    getDatasetsCount = await aDatasetHubInstance.getDatasetsCount.call(marketplaceCreator);
    assert.strictEqual(getDatasetsCount.toNumber(), 2, "check getDatasetsCount");

    getDataset = await aDatasetHubInstance.getDataset.call(marketplaceCreator, 2);
    assert.strictEqual(getDataset, createdDataset2, "check getDataset");

  });

  it("createDataset_02 : a non owner can't call createDataset", async function() {
    await Extensions.expectedExceptionPromise(() => {
        return aDatasetHubInstance.createDataset(
          "dataset Name 2",
          1,
          "dataset Params 2", {
            from: iExecCloudUser,
