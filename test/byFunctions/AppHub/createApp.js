...
contract('IexecHub', function(accounts) {
  ...
  it("createApp_01 : the owner of ( here marketplaceCreator ) can call createApp and create several App", async function() {
    ...
    isAppRegistered = await aAppHubInstance.isAppRegistered.call(createdApp);
    assert.isFalse(isAppRegistered, "check isAppRegistered false");

    getAppsCount = await aAppHubInstance.getAppsCount.call(marketplaceCreator);
    assert.strictEqual(getAppsCount.toNumber(), 0, "check getAppsCount");

    getApp = await aAppHubInstance.getApp.call(marketplaceCreator, 1);
    ...
    isAppRegistered = await aAppHubInstance.isAppRegistered.call(createdApp);
    assert.isTrue(isAppRegistered, "check isAppRegistered");

    getAppsCount = await aAppHubInstance.getAppsCount.call(marketplaceCreator);
    assert.strictEqual(getAppsCount.toNumber(), 1, "check getAppsCount");
    ...
    isAppRegistered = await aAppHubInstance.isAppRegistered.call(createdApp2);
    assert.isTrue(isAppRegistered, "check isAppRegistered");

    getAppsCount = await aAppHubInstance.getAppsCount.call(marketplaceCreator);
    assert.strictEqual(getAppsCount.toNumber(), 2, "check getAppsCount");

    getApp = await aAppHubInstance.getApp.call(marketplaceCreator, 2);
    assert.strictEqual(getApp, createdApp2, "check getApp");

  });

  it("createApp_02 : a non owner can't call createApp", async function() {
    await Extensions.expectedExceptionPromise(() => {
        return aAppHubInstance.createApp(
          "app Name",
          0,
          "app Params", {
            from: iExecCloudUser,
