...
contract('IexecHub', async (accounts) => {
	...
	it("check dapp hash", async () => {
		...
		odbtools.signObject(dapporder, dappProvider, (obj) => dapporder_hash);
		...
		assert.notEqual(await IexecClerkInstanceEthers.getDappOrderHash({ dapp: dapporder.dapp,         dappprice: dapporder.dappprice, volume: dapporder.volume,       datarestrict: dapporder.datarestrict, poolrestrict: dapporder.poolrestrict, userrestrict: dapporder.userrestrict, salt: ethers.utils.randomBytes(32), sign: dapporder.sign           }), dapporder_hash, "Check dapporder hash");
	...
	it("check data hash", async () => {
		...
		odbtools.signObject(dataorder, dataProvider, (obj) => dataorder_hash);
		...
		assert.notEqual(await IexecClerkInstanceEthers.getDataOrderHash({ data: dataorder.data,         dataprice: dataorder.dataprice, volume: dataorder.volume,       dapprestrict: dataorder.dapprestrict, poolrestrict: dataorder.poolrestrict, userrestrict: dataorder.userrestrict, salt: ethers.utils.randomBytes(32), sign: dataorder.sign           }), dataorder_hash, "Check dataorder hash");
	...
	it("check pool hash", async () => {
		...
		odbtools.signObject(poolorder, poolScheduler, (obj) => poolorder_hash);
		...
		assert.notEqual(await IexecClerkInstanceEthers.getPoolOrderHash({ pool: poolorder.pool,         poolprice: poolorder.poolprice, volume: poolorder.volume, category: poolorder.category, trust: poolorder.trust, tag: poolorder.tag, dapprestrict: poolorder.dapprestrict, datarestrict: poolorder.datarestrict, userrestrict: poolorder.userrestrict, salt: ethers.utils.randomBytes(32), sign: poolorder.sign           }), poolorder_hash, "Check poolorder hash");
	...
	it("check user hash", async () => {
		...
		odbtools.signObject(userorder, poolScheduler, (obj) => userorder_hash);
		...
		assert.notEqual(await IexecClerkInstanceEthers.getUserOrderHash({ dapp: userorder.dapp,         dappmaxprice: userorder.dappmaxprice, data: userorder.data,         datamaxprice: userorder.datamaxprice, pool: userorder.pool,         poolmaxprice: userorder.poolmaxprice, volume: userorder.volume, category: userorder.category, trust: userorder.trust, tag: userorder.tag, requester: userorder.requester,    beneficiary: userorder.beneficiary,  callback: userorder.callback, params: userorder.params, salt: ethers.utils.randomBytes(32), sign: userorder.sign           }), userorder_hash, "Check userorder hash");
