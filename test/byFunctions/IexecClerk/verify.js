...
contract('IexecHub', async (accounts) => {
	...
	it("check signature mechanism", async () => {
		...
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(user,                   entry.hash,               { v: entry.sign.v, r: entry.sign.r,             s: entry.sign.s             }, {}), "Error with the validation of signatures");
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(constants.NULL.ADDRESS, entry.hash,               { v: entry.sign.v, r: entry.sign.r,             s: entry.sign.s             }, {}), "Error with the validation of signatures");
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(iexecAdmin,             web3.utils.randomHex(32), { v: entry.sign.v, r: entry.sign.r,             s: entry.sign.s             }, {}), "Error with the validation of signatures");
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(iexecAdmin,             constants.NULL.BYTES32,   { v: entry.sign.v, r: entry.sign.r,             s: entry.sign.s             }, {}), "Error with the validation of signatures");
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(iexecAdmin,             entry.hash,               { v: 0,            r: entry.sign.r,             s: entry.sign.s             }, {}), "Error with the validation of signatures");
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(iexecAdmin,             entry.hash,               { v: entry.sign.v, r: web3.utils.randomHex(32), s: entry.sign.s             }, {}), "Error with the validation of signatures");
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(iexecAdmin,             entry.hash,               { v: entry.sign.v, r: entry.sign.r,             s: web3.utils.randomHex(32) }, {}), "Error with the validation of signatures");
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(iexecAdmin,             entry.hash,               constants.NULL.SIGNATURE,                                                      {}), "Error with the validation of signatures");
	});
	...
	it("check app hash", async () => {
		...
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(appProvider, odbtools.AppOrderStructHash({ app: constants.NULL.ADDRESS, appprice: apporder.appprice, volume: apporder.volume, tag: apporder.tag, datasetrestrict: apporder.datasetrestrict, workerpoolrestrict: apporder.workerpoolrestrict, requesterrestrict: apporder.requesterrestrict, salt: apporder.salt            }), apporder.sign,            {}));
	...
	it("check workerpool hash", async () => {
		...
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(scheduler, odbtools.WorkerpoolOrderStructHash({ workerpool: constants.NULL.ADDRESS,     workerpoolprice: workerpoolorder.workerpoolprice, volume: workerpoolorder.volume, category: workerpoolorder.category, trust: workerpoolorder.trust, tag: workerpoolorder.tag, apprestrict: workerpoolorder.apprestrict, datasetrestrict: workerpoolorder.datasetrestrict, requesterrestrict: workerpoolorder.requesterrestrict, salt: workerpoolorder.salt     }), workerpoolorder.sign,     {}));
		await odbtools.reverts(() => IexecClerkInstance.verifySignature(scheduler, odbtools.WorkerpoolOrderStructHash({ workerpool: workerpoolorder.workerpool, workerpoolprice: 0,                               volume: workerpoolorder.volume, category: workerpoolorder.category, trust: workerpoolorder.trust, tag: workerpoolorder.tag, apprestrict: workerpoolorder.apprestrict, datasetrestrict: workerpoolorder.datasetrestrict, requesterrestrict: workerpoolorder.requesterrestrict, salt: workerpoolorder.salt     }), workerpoolorder.sign,     {}));
