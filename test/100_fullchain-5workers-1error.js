...
contract('Fullchain', async (accounts) => {
	...
	before("configure", async () => {
		...
		workers = [
			...
			{ address: worker2, agent: new odbtools.MockWorker(worker2), useenclave: true,  result: "je ne veux pas travailler" },
			{ address: worker3, agent: new odbtools.MockWorker(worker3), useenclave: true,  result: "iExec the wanderer" },
			{ address: worker4, agent: new odbtools.MockWorker(worker4), useenclave: true,  result: "iExec the wanderer" },
			{ address: worker5, agent: new odbtools.MockWorker(worker5), useenclave: true,  result: "iExec the wanderer" },
		];
	...
	describe("→ summary", async () => {
		...
		it("balances", async () => {
			...
			assert.deepEqual(Object.extract(await IexecInstance.viewAccount(scheduler      ), [ 'stake', 'locked' ]).map(bn => Number(bn)), [ 1005, 0 ], "check balance");
			assert.deepEqual(Object.extract(await IexecInstance.viewAccount(worker1        ), [ 'stake', 'locked' ]).map(bn => Number(bn)), [ 1007, 0 ], "check balance");
			assert.deepEqual(Object.extract(await IexecInstance.viewAccount(worker2        ), [ 'stake', 'locked' ]).map(bn => Number(bn)), [  992, 0 ], "check balance");
			assert.deepEqual(Object.extract(await IexecInstance.viewAccount(worker3        ), [ 'stake', 'locked' ]).map(bn => Number(bn)), [ 1007, 0 ], "check balance");
			assert.deepEqual(Object.extract(await IexecInstance.viewAccount(worker4        ), [ 'stake', 'locked' ]).map(bn => Number(bn)), [ 1007, 0 ], "check balance");
			assert.deepEqual(Object.extract(await IexecInstance.viewAccount(worker5        ), [ 'stake', 'locked' ]).map(bn => Number(bn)), [ 1007, 0 ], "check balance");
