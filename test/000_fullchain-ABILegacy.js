...
contract('Fullchain', async (accounts) => {
    ...
    describe('→ pipeline', async () => {
        ...
        describe('[1] order matching', async () => {
            it('[TX] match', async () => {
                txMined = await IexecInstance.matchOrders(
                    apporder,
                    datasetorder,
                    workerpoolorder,
                    requestorder,
                ...
                dealid = web3.utils.soliditySha3(
                    {
                        t: 'bytes32',
                        v: odbtools.utils.hashRequestOrder(ERC712_domain, requestorder),
                    },
                    { t: 'uint256', v: 0 },
                );
                ...
                assert.equal(events[0].args.dealid, dealid);
                assert.equal(
                    events[0].args.appHash,
                    odbtools.utils.hashAppOrder(ERC712_domain, apporder),
                );
                assert.equal(
                    events[0].args.datasetHash,
                    odbtools.utils.hashDatasetOrder(ERC712_domain, datasetorder),
                );
                assert.equal(
                    events[0].args.workerpoolHash,
                    odbtools.utils.hashWorkerpoolOrder(ERC712_domain, workerpoolorder),
                );
                assert.equal(
                    events[0].args.requestHash,
                    odbtools.utils.hashRequestOrder(ERC712_domain, requestorder),
                );
            ...
            describe('checks', async () => {
                it('deal', async () => {
                    ...
                    assert.equal(
                        deal_pt1[0] /*dapp.pointer*/,
                        AppInstance.address,
                        'check deal (deal.dapp.pointer)',
                    );
                    assert.equal(
                        deal_pt1[0] /*dapp.pointer*/,
                        requestorder.app,
                    ...
                    assert.equal(
                        deal_pt1[1] /*dapp.owner*/,
                        appProvider.address,
                        'check deal (deal.dapp.owner)',
                    );
                    assert.equal(
                        deal_pt1[2].toNumber() /*dapp.price*/,
                        apporder.appprice,
                        'check deal (deal.dapp.price)',
                    );
                    assert.isAtMost(
                        deal_pt1[2].toNumber() /*dapp.price*/,
                    ...
                    assert.equal(
                        deal_pt1[3] /*data.pointer*/,
                        DatasetInstance.address,
                        'check deal (deal.data.pointer)',
                    );
                    assert.equal(
                        deal_pt1[3] /*data.pointer*/,
                    ...
                    assert.equal(
                        deal_pt1[4] /*data.owner*/,
                        datasetProvider.address,
                        'check deal (deal.data.owner)',
                    );
                    assert.equal(
                        deal_pt1[5].toNumber() /*data.price*/,
                        datasetorder.datasetprice,
                        'check deal (deal.data.price)',
                    );
                    assert.isAtMost(
                        deal_pt1[5].toNumber() /*data.price*/,
                    ...
                    assert.equal(
                        deal_pt1[6] /*pool.pointer*/,
                        WorkerpoolInstance.address,
                        'check deal (deal.pool.pointer)',
                    );
                    ...
                    assert.equal(
                        deal_pt1[7] /*pool.owner*/,
                        scheduler.address,
                        'check deal (deal.pool.owner)',
                    );
                    assert.equal(
                        deal_pt1[8].toNumber() /*pool.price*/,
                        workerpoolorder.workerpoolprice,
                        'check deal (deal.pool.price)',
                    );
                    assert.isAtMost(
                        deal_pt1[8].toNumber() /*pool.price*/,
                ...
                it('config', async () => {
                    ...
                    assert.equal(
                        config[5].toNumber(),
                        5,
                        'check config (config.schedulerRewardRatio)',
                    );
                });
        ...
        describe('[2] initialization', async () => {
            it('[TX] initialize', async () => {
                ...
                taskid = web3.utils.soliditySha3(
                    { t: 'bytes32', v: dealid },
                    { t: 'uint256', v: 0 },
                );
            ...
            describe('checks', async () => {
                it('task', async () => {
                    ...
                    assert.equal(
                        task[3].toNumber(),
                        (await IexecInstance.viewCategoryABILegacy(requestorder.category))[2],
                    );
        ...
        describe('[3] contribute', async () => {
            it('authorization signature', async () => {
                for (w of workers) {
                    ...
                    const [auth, secret] = w.useenclave
            ...
            it('run', async () => {
                ...
                for (w of workers) {
                    results[w.agent.address] = await w.agent.run(
                        authorizations[w.agent.address],
                        secrets[w.agent.address],
                        w.result,
                    );
                }
            });
            ...
            describe('checks', async () => {
                it('contribution', async () => {
                    for (w of workers) {
                        contribution = await IexecInstance.viewContributionABILegacy(
                            taskid,
                            w.agent.address,
                        );
                        assert.equal(
                            contribution[0],
                            constants.ContributionStatusEnum.CONTRIBUTED,
                            'check contribution (contribution.status)',
                        );
                        assert.equal(
                            contribution[1],
                            results[w.agent.address].hash,
                            'check contribution (contribution.resultHash)',
                        );
                        assert.equal(
                            contribution[2],
                            results[w.agent.address].seal,
                            'check contribution (contribution.resultSeal)',
                        );
                        assert.equal(
                            contribution[3],
                            authorizations[w.agent.address].enclave,
                            'check contribution (contribution.enclaveChallenge)',
                        );
                    }
                });
            });
    ...
    describe('→ summary', async () => {
        ...
        it('balances - extra', async () => {
            ...
            for (agent of [
                ...
                assert.deepEqual(
                    await agent.viewAccount(),
                    [
                        Number(await IexecInstance.balanceOf(agent.address)),
                        Number(await IexecInstance.frozenOf(agent.address)),
                    ],
                    'check balance',
                );
            }
