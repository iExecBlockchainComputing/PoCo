...
contract('ERC1154: resultFor', async (accounts) => {
    ...
    describe('→ pipeline', async () => {
        ...
        describe('[2] initialization', async () => {
            it('[TX] initialize', async () => {
                tasks[0] = web3.utils.soliditySha3(
                    { t: 'bytes32', v: deals[0] },
                    { t: 'uint256', v: 0 },
                ); // uninitialized
                tasks[1] = tools.extractEvents(
                    await IexecInstance.initialize(deals[0], 1, { from: scheduler.address }),
                    IexecInstance.address,
                    'TaskInitialize',
                )[0].args.taskid; // finalized
                tasks[2] = tools.extractEvents(
                    await IexecInstance.initialize(deals[0], 2, { from: scheduler.address }),
                    IexecInstance.address,
                    'TaskInitialize',
                )[0].args.taskid; // initialized
                tasks[3] = tools.extractEvents(
                    await IexecInstance.initialize(deals[0], 3, { from: scheduler.address }),
                    IexecInstance.address,
                    'TaskInitialize',
                )[0].args.taskid; // contributions
                tasks[4] = tools.extractEvents(
                    await IexecInstance.initialize(deals[0], 4, { from: scheduler.address }),
                    IexecInstance.address,
                    'TaskInitialize',
                )[0].args.taskid; // consensus
                tasks[5] = tools.extractEvents(
                    await IexecInstance.initialize(deals[0], 5, { from: scheduler.address }),
                    IexecInstance.address,
                    'TaskInitialize',
                )[0].args.taskid; // reveal
            });
        });
        ...
        describe('[3] contribute', async () => {
            it('[TX] contribute', async () => {
                ...
                await sendContribution(
                    ...
                    'true',
                    false,
                    web3.utils.utf8ToHex('callback-5'),
                );
            });
        });
            ...
            it('[TX] reveal', async () => {
                ...
                await IexecInstance.reveal(
                    ...
                    odbtools.utils.hashByteResult(
                        ...
                        web3.utils.soliditySha3({
                            t: 'bytes',
                            v: web3.utils.utf8ToHex('callback-5'),
                        }),
                    ).digest,
                    { from: worker1.address },
                );
        ...
        describe('[5] finalization', async () => {
            it('[TX] finalize', async () => {
                await IexecInstance.finalize(
                    tasks[5],
                    web3.utils.utf8ToHex('aResult 5'),
                    web3.utils.utf8ToHex('callback-5'),
                    { from: scheduler.address },
                );
            });
    ...
    describe('→ result for', async () => {
        ...
        describe('finalized', async () => {
            it('valid', async () => {
                assert.equal(
                    await IexecInstance.resultFor(tasks[5]),
                    web3.utils.utf8ToHex('callback-5'),
                    'resultFor returned incorrect value',
                );
            });
        });
    });
