    ...
    describe('→ pipeline', async () => {
        ...
        describe('[1] order matching', async () => {
            it('[TX] match', async () => {
                ...
                dealids = await odbtools.utils.requestToDeal(
                    IexecInstance,
                    odbtools.utils.hashRequestOrder(ERC712_domain, requestorder),
                );
        ...
        describe('[2] initialization', async () => {
            it('[TX] initialize task', async () => {
                txMined = await IexecInstance.initialize(dealids[0], 0, {
                    from: scheduler.address,
                });
                ...
                txMined = await IexecInstance.initialize(dealids[0], 1, {
                    from: scheduler.address,
                });
                gasReceipt.push(['initialize', txMined.receipt.gasUsed]);
                ...
                txMined = await IexecInstance.initialize(dealids[1], 2, {
                    from: scheduler.address,
                });
                gasReceipt.push(['initialize', txMined.receipt.gasUsed]);
        ...
        describe('[3] contribute', async () => {
            it('authorization signature', async () => {
                for (i in tasks)
                    for (w of tasks[i].workers) {
                        const preauth = await scheduler.signPreAuthorization(
                            tasks[i].taskid,
                            w.agent.address,
                        );
            ...
            it('run', async () => {
                for (i in tasks) {
                    tasks[i].consensus = odbtools.utils.hashConsensus(
                        tasks[i].taskid,
                        tasks[i].consensus,
                    );
                    for (w of tasks[i].workers) {
                        tasks[i].results[w.agent.address] = await w.agent.run(
                            tasks[i].authorizations[w.agent.address],
                            tasks[i].secrets[w.agent.address],
                            w.result,
                        );
                    }
                }
            ...
            describe('task 1', async () => {
                ...
                describe('checks', async () => {
                    it('task', async () => {
                        ...
                        assert.equal(
                            Number(task.timeref),
                            (await IexecInstance.viewCategory(requestorder.category))
                                .workClockTimeRef,
                        );
                        ...
                        assert.deepEqual(
                            ...
                            tasks[0].workers.map((w) => w.agent.address),
                        );
                    ...
                    it('balances', async () => {
                        assert.deepEqual(
                            await appProvider.viewAccount(),
                            [0 + 3 + 0 + 0, 0],
                            'check balance',
                        );
                        assert.deepEqual(
                            await datasetProvider.viewAccount(),
                            [0 + 1 + 0 + 0, 0],
                            'check balance',
                        );
                        assert.deepEqual(
                            await scheduler.viewAccount(),
                            [1000 + 1 - 4 - 7, 0 + 4 + 7],
                            'check balance',
                        );
                        assert.deepEqual(
                            await worker1.viewAccount(),
                            [1000 + 7 - 8, 0 + 8],
                            'check balance',
                        );
                        assert.deepEqual(
                            await worker2.viewAccount(),
                            [1000 + 7 - 5 - 8, 0 + 5 + 8],
                            'check balance',
                        );
                        assert.deepEqual(
                            await worker3.viewAccount(),
                            [1000 - 5 - 8, 0 + 5 + 8],
                            'check balance',
                        );
                        assert.deepEqual(
                            await worker4.viewAccount(),
                            [1000 - 8, 0 + 8],
                            'check balance',
                        );
                        assert.deepEqual(
                            await worker5.viewAccount(),
                            [1000 - 8, 0 + 8],
                            'check balance',
                        );
                        assert.deepEqual(
                            await user.viewAccount(),
                            [1000 - 19 - 19 - 29, 0 + 19 + 29],
                            'check balance',
                        );
                    });
            ...
            describe('task 2', async () => {
                ...
                describe('checks', async () => {
                    it('task', async () => {
                        ...
                        assert.deepEqual(
                            task.contributors.map((a) => a),
                            tasks[1].workers.map((w) => w.agent.address),
                        );
            ...
            describe('task 3', async () => {
                ...
                describe('checks', async () => {
                    it('task', async () => {
                        ...
                        assert.deepEqual(
                            task.contributors.map((a) => a),
                            tasks[2].workers.map((w) => w.agent.address),
                        );
