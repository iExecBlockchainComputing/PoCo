...
contract('Fullchain', async (accounts) => {
	...
	describe("→ pipeline", async () => {
		...
		describe("[1] order matching", async () => {
			it("[TX] match", async () => {
				const txsMined = [
					await IexecInstance.matchOrders(apporder, datasetorder, workerpoolorder1, requestorder, { from: user.address }),
					await IexecInstance.matchOrders(apporder, datasetorder, workerpoolorder2, requestorder, { from: user.address }),
				];
