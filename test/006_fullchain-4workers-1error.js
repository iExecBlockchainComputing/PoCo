...
contract('IexecHub', async (accounts) => {
	...
	it(">> reveal", async () => {
		...
		if (results[w.address].contribution.hash == consensus.contribution.hash)
		{
			...
			assert.equal(events[0].args.hash,   results[w.address].base, "check result");
		}
	});
	...
	it("[Finalized] Check task", async () => {
		...
		assert.equal    (task.revealCounter.toNumber(),     workers.length - 1,                 "check task (task.revealCounter)"    );
		assert.equal    (task.winnerCounter.toNumber(),     workers.length - 1,                 "check task (task.winnerCounter)"    );
	...
	it("[Finalized] Check balances", async () => {
		...
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker1  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 1000, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker2  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 1000, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker3  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [  982, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker4  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 1000, 10 ], "check balance");
