...
contract('IexecHub', async (accounts) => {
	...
	it("[Finalized] Check task", async () => {
		...
		assert.equal    (Number(task.revealCounter),           workers.length - 1,                 "check task (task.revealCounter)"    );
		assert.equal    (Number(task.winnerCounter),           workers.length - 1,                 "check task (task.winnerCounter)"    );
	...
	it("[Finalized] Check balances", async () => {
		balance = await IexecClerkInstance.viewAccount(datasetProvider); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [    1,  0 ], "check balance");
		balance = await IexecClerkInstance.viewAccount(appProvider    ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [    3,  0 ], "check balance");
		balance = await IexecClerkInstance.viewAccount(scheduler      ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1003,  0 ], "check balance");
		balance = await IexecClerkInstance.viewAccount(worker1        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1000, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccount(worker2        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [  982, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccount(worker3        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1000, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccount(worker4        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1000, 10 ], "check balance");
