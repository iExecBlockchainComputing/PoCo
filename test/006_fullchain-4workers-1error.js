...
contract('IexecHub', async (accounts) => {
	...
	//assert.equal    (workorder.revealDeadline,                         "",                                      "check workorder (workorder.revealDeadline)"   );
		assert.equal    (workorder.revealCounter,                          0,                                       "check workorder (workorder.revealCounter)"    );
		assert.equal    (workorder.winnerCounter,                          workers.length - 1,                      "check workorder (workorder.winnerCounter)"    );
		assert.deepEqual(workorder.contributors.map(a => a.toLowerCase()), workers.map(x => x.address),             "check workorder (workorder.contributors)"     );
	});
	...
	//assert.equal    (workorder.consensusDeadline,                      "",                                      "check workorder (workorder.consensusDeadline)");
	//assert.equal    (workorder.revealDeadline,                         "",                                      "check workorder (workorder.revealDeadline)"   );
		assert.equal    (workorder.revealCounter,                          workers.length - 1,                      "check workorder (workorder.revealCounter)"    );
		assert.equal    (workorder.winnerCounter,                          workers.length - 1,                      "check workorder (workorder.winnerCounter)"    );
		assert.deepEqual(workorder.contributors.map(a => a.toLowerCase()), workers.map(x => x.address),             "check workorder (workorder.contributors)"     );
	});
	...
	it("Check balances - After", async () => {
		...
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker1  )).map(x => x.toNumber()), [ 1000, 10 ], "check balance"); // +10
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker2  )).map(x => x.toNumber()), [ 1000, 10 ], "check balance"); // +10
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker3  )).map(x => x.toNumber()), [  982, 10 ], "check balance"); //  -8
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker4  )).map(x => x.toNumber()), [ 1000, 10 ], "check balance"); // +10
