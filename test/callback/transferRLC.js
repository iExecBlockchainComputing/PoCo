...
var SMART_CONTRACT_ADDRESS = process.argv[3] || "0x091233035DCB12ae5A4A4b7fB144D3C5189892E1";//RLC contract
...
async function getAbiContent() {
  try {
    var abiFileContent = await readFileAsync("../../deployed/contracts/RLC.json");
...
async function run() {
  try {
    ...
    web3.eth.getTransactionCountPromise(MSG_SENDER).timeout(2000).then(function(currentNonce) {
      ...
      try {
        contract.methods.transfer('0xeae99b010f8b8852ab47ba883f4c5157633c5ac6'/*IexecAPI contract*/,200).send({
            from: MSG_SENDER,
            gas: "4000000",
