...
describe('Poco', async () => {
    ...
    describe('Balance of', function () {
        it('Should get balance when account has no tokens', async function () {
            expect(await iexecPoco.balanceOf(anyone.address)).equal(0);
        });
        it('Should get balance when account has some tokens', async function () {
    ...
    describe('Transfer', function () {
        ...
        it('Should not transfer when sender balance too low', async function () {
            await expect(
    ...
    describe('Approve and call', function () {
        ...
        it('Should not approve and call from the zero address', async function () {
            await expect(
                iexecPoco
    ...
    describe('Transfer from', function () {
        it('Should transferFrom some tokens', async function () {
        ...
        it('Should not transferFrom when owner is the zero address', async function () {
            await expect(
                iexecPocoAsSpender.transferFrom(zeroAddress, spender.address, value),
            ).to.be.revertedWith('ERC20: transfer from the zero address');
        });
        it('Should not transferFrom to the zero address', async function () {
            await expect(
                iexecPocoAsSpender.transferFrom(holder.address, zeroAddress, value),
            ).to.be.revertedWith('ERC20: transfer to the zero address');
        });
        it('Should not transferFrom when owner balance too low', async function () {
            await expect(
                iexecPocoAsSpender.transferFrom(holder.address, spender.address, value + 1),
            ).to.be.revertedWithoutReason();
        });
        it('Should not transferFrom when spender allowance too low', async function () {
    ...
    describe('Increase allowance', function () {
        ...
        it('Should not increase allowance from the zero address', async function () {
            await expect(
                iexecPoco.connect(zeroAddressSigner).increaseAllowance(spender.address, value),
            ).to.be.revertedWith('ERC20: approve from the zero address');
        });
        it('Should not increase allowance for the zero address', async function () {
            await expect(
    ...
    describe('Decrease allowance', function () {
        ...
        it('Should not decrease allowance of a value greater than old allowance', async function () {
            await expect(
