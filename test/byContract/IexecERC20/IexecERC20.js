...
contract('ERC20', async (accounts) => {
    ...
    beforeEach(async function () {
        const txsMined = [
            await IexecInstance.withdraw(await IexecInstance.balanceOf(initialHolder), {
                from: initialHolder,
                gas: constants.AMOUNT_GAS_PROVIDED,
            }),
            await IexecInstance.withdraw(await IexecInstance.balanceOf(recipient), {
                from: recipient,
                gas: constants.AMOUNT_GAS_PROVIDED,
            }),
            await IexecInstance.withdraw(await IexecInstance.balanceOf(anotherAccount), {
                from: anotherAccount,
                gas: constants.AMOUNT_GAS_PROVIDED,
            }),
            await IexecInstance.approve(initialHolder, 0, {
                from: initialHolder,
                gas: constants.AMOUNT_GAS_PROVIDED,
            }),
            await IexecInstance.approve(initialHolder, 0, {
            ...
            await IexecInstance.approve(recipient, 0, {
                from: initialHolder,
                gas: constants.AMOUNT_GAS_PROVIDED,
            }),
            await IexecInstance.approve(recipient, 0, {
            ...
            await IexecInstance.approve(anotherAccount, 0, {
                from: initialHolder,
                gas: constants.AMOUNT_GAS_PROVIDED,
            }),
            await IexecInstance.approve(anotherAccount, 0, {
        ...
        assert.isBelow(
            txsMined[0].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[1].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[2].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[3].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[4].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[5].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[6].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[7].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[8].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[9].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[10].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        assert.isBelow(
            txsMined[11].receipt.gasUsed,
            constants.AMOUNT_GAS_PROVIDED,
            'should not use all gas',
        );
        ...
        switch (DEPLOYMENT.asset) {
            ...
            case 'Token':
                txMined = await RLCInstance.approveAndCall(
                    ...
                    initialSupply,
                    '0x',
                    { from: initialHolder, gas: constants.AMOUNT_GAS_PROVIDED },
                );
                assert.isBelow(
                    txMined.receipt.gasUsed,
                    constants.AMOUNT_GAS_PROVIDED,
    ...
    describe('approveAndCall', async () => {
        it('accepted by spender', async () => {
            await IexecInstance.approveAndCall(TestReceiverInstance.address, 10, '0x', {
                from: initialHolder,
            });
        ...
        it('rejected by spender', async () => {
            await expectRevert(
                IexecInstance.approveAndCall(TestReceiverInstance.address, 0, '0x', {
                    from: initialHolder,
                }),
                'approval-refused',
            );
