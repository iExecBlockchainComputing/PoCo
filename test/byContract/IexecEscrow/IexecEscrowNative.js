...
contract('EscrowToken', async (accounts) => {
	...
	describe("deposit", async () => {
		it("success", async () => {
			...
			txMined = await IexecInstance.deposit({ from: accounts[0], value: 100, gas: constants.AMOUNT_GAS_PROVIDED });
	...
	describe("depositForArray", async () => {
			...
			describe("amounts.length > target.length", () => {
				it("reverts", async () => {
					...
					await expectRevert.unspecified(IexecInstance.depositForArray(
						[ 100, 100, ],
						[ accounts[3] ],
						{ from: accounts[2], value: 200, gas: constants.AMOUNT_GAS_PROVIDED }
					));
		...
		describe("length match", () => {
			it("success", async () => {
				...
				assert.equal(await web3.eth.getBalance(IexecInstance.address), 400);
				...
				assert.deepEqual(Object.extract(await IexecInstance.viewAccount(accounts[4]), [ 'stake', 'locked' ]).map(bn => Number(bn)), [ 100, 0 ], "check balance");
			});
			...
			it("emit events", async () => {
				...
				assert.equal(events[1].args.to,    accounts[4]);
		...
		describe("excess value", () => {
			it("success", async () => {
				...
				txMined = await IexecInstance.depositForArray(
					...
					{ from: accounts[5], value: 250, gas: constants.AMOUNT_GAS_PROVIDED }
				);

				assert.equal(await web3.eth.getBalance(IexecInstance.address), 650);
				assert.deepEqual(Object.extract(await IexecInstance.viewAccount(accounts[5]), [ 'stake', 'locked' ]).map(bn => Number(bn)), [  50, 0 ], "check balance");
			...
			it("emit events", async () => {
				events = extractEvents(txMined, IexecInstance.address, "Transfer");
				assert.equal(events[0].args.from,  constants.NULL.ADDRESS);
				assert.equal(events[0].args.to,    accounts[6]);
				assert.equal(events[0].args.value, 100);
				assert.equal(events[1].args.from,  constants.NULL.ADDRESS);
				assert.equal(events[1].args.to,    accounts[7]);
				assert.equal(events[1].args.value, 100);
				assert.equal(events[2].args.from,  constants.NULL.ADDRESS);
				assert.equal(events[2].args.to,    accounts[5]);
