...
contract('ENSIntegration', async (accounts) => {
    ...
    describe('Initial state (migration)', async () => {
        it('lookup', async () => {
            ...
            assert.equal(
                await enstools.lookup(DatasetRegistryInstance.address),
                'datasets.v5.iexec.eth',
            );
            assert.equal(
                await enstools.lookup(WorkerpoolRegistryInstance.address),
        ...
        it('resolve', async () => {
            ...
            assert.equal(
                await enstools.resolve('datasets.v5.iexec.eth'),
                DatasetRegistryInstance.address,
            );
            assert.equal(
                await enstools.resolve('workerpools.v5.iexec.eth'),
    ...
    describe('Reverse register', async () => {
        describe('unauthorized', async () => {
            it('reverts', async () => {
                await expectRevert(
                    IexecInstance.setName(ENSInstance.address, 'wrong.domain.eth', {
                        from: user.address,
                    }),
        ...
        describe('authorized', async () => {
            it('success', async () => {
                await IexecInstance.setName(ENSInstance.address, 'test.namespace.eth', {
