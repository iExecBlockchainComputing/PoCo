...
async function deployBoostFixture() {
    const [admin, requester, beneficiary, appProvider, datasetProvider,
        scheduler, worker1] = await ethers.getSigners();
    const iexecPocoBoostInstance: IexecPocoBoostDelegate =
        await new IexecPocoBoostDelegate__factory()
            .connect(admin)
            .deploy().then(instance => instance.deployed())
    return {
        iexecPocoBoostInstance, admin, requester, beneficiary, appProvider,
        datasetProvider, scheduler, worker1
    };
}

describe("Match orders boost", function () {
    it("Should match orders", async function () {
        const { iexecPocoBoostInstance, requester, beneficiary,
            appProvider } = await loadFixture(deployBoostFixture);

        const appInstance = await smock.mock<App__factory>("App")
        ...
        await expect(iexecPocoBoostInstance
            .matchOrdersBoost(requestOrder, appOrder))
