    ...
    describe('Registry', async () => {
        it('cannot reinitialize', async () => {
            ...
            await expectRevert.unspecified(
                DatasetRegistryInstance.initialize(constants.NULL.ADDRESS),
            );
            await expectRevert.unspecified(
                WorkerpoolRegistryInstance.initialize(constants.NULL.ADDRESS),
            );
        });
        it('baseURI', async () => {
            const chainid = await web3.eth.net.getId();
            assert.equal(
                await AppRegistryInstance.baseURI(),
                `https://nfts-metadata.iex.ec/app/${chainid}/`,
            );
            assert.equal(
                await DatasetRegistryInstance.baseURI(),
                `https://nfts-metadata.iex.ec/dataset/${chainid}/`,
            );
            assert.equal(
                await WorkerpoolRegistryInstance.baseURI(),
            ...
            .map((_, i) => {
                describe(`app #${i}`, async () => {
                    it('creation', async () => {
                        ...
                        const predictedAddress = tools.create2(
                        ...
                        assert.equal(
                            await AppRegistryInstance.predictApp(
                                appProvider.address,
                                'App #' + i,
                                'DOCKER',
                                constants.MULTIADDR_BYTES,
                    ...
                    it('content', async () => {
                        ...
                        assert.equal(
                            await AppInstances[i].m_appMultiaddr(),
                            constants.MULTIADDR_BYTES,
                        );
                        assert.equal(
                            await AppInstances[i].m_appChecksum(),
                            web3.utils.keccak256('Content of app #' + i),
                        );
                    ...
                    it('token details', async () => {
                        assert.equal(
                            await AppRegistryInstance.ownerOf(AppInstances[i].address),
                            appProvider.address,
                        );
                        assert.equal(
                            await AppRegistryInstance.balanceOf(appProvider.address),
                            i + 1,
                        );
                        assert.isTrue(
                            await AppRegistryInstance.isRegistered(AppInstances[i].address),
                        );
                        assert.equal(
                            tools.BN2Address(
                                await AppRegistryInstance.tokenOfOwnerByIndex(
                                    appProvider.address,
                                    i,
                                ),
                            ),
                            AppInstances[i].address,
                        );
                        assert.equal(
                            await AppRegistryInstance.tokenURI(AppInstances[i].address),
                            (await AppRegistryInstance.baseURI()) +
                                web3.utils.toBN(AppInstances[i].address),
                        );
                    });
            ...
            .map((_, i) => {
                describe(`dataset #${i}`, async () => {
                    it('creation', async () => {
                        ...
                        assert.equal(
                            await DatasetRegistryInstance.predictDataset(
                    ...
                    it('content', async () => {
                        assert.equal(
                            await DatasetInstances[i].registry(),
                            DatasetRegistryInstance.address,
                        );
                        ...
                        assert.equal(
                            await DatasetInstances[i].m_datasetMultiaddr(),
                            constants.MULTIADDR_BYTES,
                        );
                        assert.equal(
                            await DatasetInstances[i].m_datasetChecksum(),
                            web3.utils.keccak256('Content of dataset #' + i),
                        );
                    });
                    ...
                    it('token details', async () => {
                        assert.equal(
                            await DatasetRegistryInstance.ownerOf(DatasetInstances[i].address),
                            datasetProvider.address,
                        );
                        assert.equal(
                            await DatasetRegistryInstance.balanceOf(datasetProvider.address),
                            i + 1,
                        );
                        assert.isTrue(
                            await DatasetRegistryInstance.isRegistered(DatasetInstances[i].address),
                        );
                        assert.equal(
                            tools.BN2Address(
                                await DatasetRegistryInstance.tokenOfOwnerByIndex(
                                    datasetProvider.address,
                                    i,
                                ),
                            ),
                            DatasetInstances[i].address,
                        );
                        assert.equal(
                            await DatasetRegistryInstance.tokenURI(DatasetInstances[i].address),
                            (await DatasetRegistryInstance.baseURI()) +
                                web3.utils.toBN(DatasetInstances[i].address),
                        );
                    });
            ...
            .map((_, i) => {
                describe(`workerpool #${i}`, async () => {
                    it('creation', async () => {
                        ...
                        assert.equal(
                            await WorkerpoolRegistryInstance.predictWorkerpool(
                                scheduler.address,
                                'Workerpool #' + i,
                            ),
                            predictedAddress,
                    ...
                    it('content', async () => {
                        assert.equal(
                            await WorkerpoolInstances[i].registry(),
                            WorkerpoolRegistryInstance.address,
                        );
                        assert.equal(await WorkerpoolInstances[i].owner(), scheduler.address);
                        assert.equal(
                            await WorkerpoolInstances[i].m_workerpoolDescription(),
                            'Workerpool #' + i,
                        );
                        assert.equal(await WorkerpoolInstances[i].m_workerStakeRatioPolicy(), 30);
                        assert.equal(
                            await WorkerpoolInstances[i].m_schedulerRewardRatioPolicy(),
                            1,
                        );
                    });
                    ...
                    it('token details', async () => {
                        assert.equal(
                            await WorkerpoolRegistryInstance.ownerOf(
                                WorkerpoolInstances[i].address,
                            ),
                            scheduler.address,
                        );
                        assert.equal(
                            await WorkerpoolRegistryInstance.balanceOf(scheduler.address),
                            i + 1,
                        );
                        assert.isTrue(
                            await WorkerpoolRegistryInstance.isRegistered(
                                WorkerpoolInstances[i].address,
                            ),
                        );
                        assert.equal(
                            tools.BN2Address(
                                await WorkerpoolRegistryInstance.tokenOfOwnerByIndex(
                                    scheduler.address,
                                    i,
                                ),
                            ),
                        ...
                        assert.equal(
                            await WorkerpoolRegistryInstance.tokenURI(
                                WorkerpoolInstances[i].address,
                            ),
                            (await WorkerpoolRegistryInstance.baseURI()) +
                                web3.utils.toBN(WorkerpoolInstances[i].address),
                        );
                    });
