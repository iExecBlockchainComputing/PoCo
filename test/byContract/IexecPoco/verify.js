    ...
    it('check signature mechanism', async () => {
        ...
        assert.isTrue(
            await IexecInstance.verifyPresignatureOrSignature(
                iexecAdmin.address,
                entry.hash,
                tools.compactSignature(entry.sign),
            ),
        );
        assert.isFalse(
            await IexecInstance.verifyPresignatureOrSignature(user.address, entry.hash, entry.sign),
        );
        assert.isFalse(
            await IexecInstance.verifyPresignatureOrSignature(
    ...
    it('check app hash', async () => {
        ...
        assert.isTrue(
            await IexecInstance.verifySignature(
                appProvider.address,
                odbtools.utils.hashAppOrder(ERC712_domain, apporder),
                tools.compactSignature(apporder.sign),
            ),
        );
        assert.isTrue(
            await IexecInstance.verifySignature(
                appProvider.address,
                odbtools.utils.hashAppOrder(ERC712_domain, {
                    app: apporder.app,
                    appprice: apporder.appprice,
                    volume: apporder.volume,
                    tag: apporder.tag,
                    datasetrestrict: apporder.datasetrestrict,
                    workerpoolrestrict: apporder.workerpoolrestrict,
                    requesterrestrict: apporder.requesterrestrict,
                    salt: apporder.salt,
                }),
                apporder.sign,
            ),
    ...
    it('check dataset hash', async () => {
        ...
        assert.isTrue(
            await IexecInstance.verifySignature(
                datasetProvider.address,
                odbtools.utils.hashDatasetOrder(ERC712_domain, datasetorder),
                tools.compactSignature(datasetorder.sign),
            ),
        );
        assert.isTrue(
            await IexecInstance.verifySignature(
                datasetProvider.address,
                odbtools.utils.hashDatasetOrder(ERC712_domain, {
                    dataset: datasetorder.dataset,
                    datasetprice: datasetorder.datasetprice,
                    volume: datasetorder.volume,
                    tag: datasetorder.tag,
                    apprestrict: datasetorder.apprestrict,
                    workerpoolrestrict: datasetorder.workerpoolrestrict,
                    requesterrestrict: datasetorder.requesterrestrict,
                    salt: datasetorder.salt,
                }),
                datasetorder.sign,
            ),
        );
    ...
    it('check workerpool hash', async () => {
        ...
        assert.isTrue(
            await IexecInstance.verifySignature(
                scheduler.address,
                odbtools.utils.hashWorkerpoolOrder(ERC712_domain, workerpoolorder),
                tools.compactSignature(workerpoolorder.sign),
            ),
        );
        assert.isTrue(
            await IexecInstance.verifySignature(
                scheduler.address,
                odbtools.utils.hashWorkerpoolOrder(ERC712_domain, {
                    workerpool: workerpoolorder.workerpool,
                    workerpoolprice: workerpoolorder.workerpoolprice,
                    volume: workerpoolorder.volume,
                    category: workerpoolorder.category,
                    trust: workerpoolorder.trust,
                    tag: workerpoolorder.tag,
                    apprestrict: workerpoolorder.apprestrict,
                    datasetrestrict: workerpoolorder.datasetrestrict,
                    requesterrestrict: workerpoolorder.requesterrestrict,
                    salt: workerpoolorder.salt,
                }),
                workerpoolorder.sign,
            ),
        );
    ...
    it('check request hash', async () => {
        ...
        assert.isTrue(
            await IexecInstance.verifySignature(
                user.address,
                odbtools.utils.hashRequestOrder(ERC712_domain, requestorder),
                tools.compactSignature(requestorder.sign),
            ),
        );
        assert.isTrue(
            await IexecInstance.verifySignature(
                user.address,
                odbtools.utils.hashRequestOrder(ERC712_domain, {
                    app: requestorder.app,
                    appmaxprice: requestorder.appmaxprice,
                    dataset: requestorder.dataset,
                    datasetmaxprice: requestorder.datasetmaxprice,
                    workerpool: requestorder.workerpool,
                    workerpoolmaxprice: requestorder.workerpoolmaxprice,
                    volume: requestorder.volume,
                    category: requestorder.category,
                    trust: requestorder.trust,
                    tag: requestorder.tag,
                    requester: requestorder.requester,
                    beneficiary: requestorder.beneficiary,
                    callback: requestorder.callback,
                    params: requestorder.params,
                    salt: requestorder.salt,
                }),
                requestorder.sign,
            ),
        );
