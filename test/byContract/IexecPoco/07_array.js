...
contract('Poco', async (accounts) => {
    ...
    it('[setup] Initialization', async () => {
        ...
        tasks[2] = tools.extractEvents(
            ...
            'TaskInitialize',
        )[0].args.taskid; // initialized
        tasks[3] = tools.extractEvents(
            txMined,
            IexecInstance.address,
            'TaskInitialize',
        )[1].args.taskid; // contributions
        tasks[4] = tools.extractEvents(
            txMined,
            IexecInstance.address,
            'TaskInitialize',
        )[2].args.taskid; // consensus
        tasks[5] = tools.extractEvents(
            txMined,
            IexecInstance.address,
            'TaskInitialize',
        )[3].args.taskid; // reveal
        tasks[6] = tools.extractEvents(
            txMined,
            IexecInstance.address,
            'TaskInitialize',
        )[4].args.taskid; // finalized
    ...
    it('[setup] Reveal', async () => {
        await IexecInstance.reveal(tasks[5], odbtools.utils.hashResult(tasks[5], 'true').digest, {
    ...
    it('[setup] Finalize', async () => {
        await IexecInstance.finalize(tasks[6], web3.utils.utf8ToHex('aResult 6'), '0x', {
            from: scheduler.address,
        });
    });
    ...
    it('[7.7a] Claim - Error (soon #6)', async () => {
        await expectRevert.unspecified(
            IexecInstance.initializeAndClaimArray([deals[1], deals[1], deals[1]], [7, 8, 9], {
                from: user.address,
            }),
        );
    });
    ...
    it('[7.1b] Claim - Correct', async () => {
        ...
        await IexecInstance.initializeArray([deals[1]], [1], { from: user.address });
        // claim
