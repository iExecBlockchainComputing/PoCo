...
describe('IexecPoco2#finalize', async () => {
    ...
    [
        {
            testInfo: 'Get 10% of the kitty when these 10% are greater than MIN_KITTY', // (MIN_KITTY=1RLC)
        ...
        {
            testInfo: 'Get at least MIN_KITTY if available',
        ...
        {
            testInfo: 'Get all kitty when the kitty is lower than MIN_KITTY',
        ...
        it(`Should finalize task with scheduler kitty part reward (${testInfo})`, async () => {
            // Fill kitty
            const kittyAddress = await iexecPoco.kitty_address();
            ...
            const kittyFilling = await iexecWrapper.signAndMatchOrders(
                ...buildOrders({
                    assets: ordersAssets,
                    requester: requester.address,
                    prices: {
            ...
            await iexecPoco
                .initialize(kittyFilling.dealId, kittyFilling.taskIndex)
                .then((tx) => tx.wait());
            const kittyFrozenBeforeClaim = (await iexecPoco.frozenOf(kittyAddress)).toNumber();
            const task = await iexecPoco.viewTask(kittyFilling.taskId);
            await time.setNextBlockTimestamp(task.finalDeadline);
            await expect(iexecPoco.claim(kittyFilling.taskId))
                .to.emit(iexecPoco, 'Transfer')
                .withArgs(iexecPoco.address, kittyAddress, kittyFillingSchedulerTaskStake)
                .to.emit(iexecPoco, 'Reward')
                .withArgs(kittyAddress, kittyFillingSchedulerTaskStake, kittyFilling.taskId)
                .to.emit(iexecPoco, 'Transfer')
                ...
                .to.changeEtherBalances([iexecPoco, kittyAddress], [0, 0]);
            ...
            await expect(iexecPocoAsScheduler.finalize(taskId, results, '0x'))
                ...
                .to.emit(iexecPoco, 'Transfer')
                .withArgs(iexecPoco.address, scheduler.address, expectedSchedulerKittyPartReward)
                .to.emit(iexecPoco, 'Reward')
