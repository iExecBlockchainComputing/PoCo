...
contract('Poco', async (accounts) => {
        ...
        await expectRevert.unspecified(
            sendContribution(
                ...
                await odbtools.utils.signAuthorization(
                    { worker: __worker.address, taskid: __taskid, enclave: __enclave.address },
                    scheduler.wallet,
                ),
                __enclave.address,
            ),
    ...
    it('[2.2][TAG] Contribute - Correct (sgx)', async () => {
        ...
        txMined = await sendContribution(
            __taskid,
            __worker,
            await odbtools.utils.signContribution(
                odbtools.utils.sealResult(__taskid, __raw, __worker.address),
                __enclave,
            ),
            await odbtools.utils.signAuthorization(
        ...
        assert.equal(
            events[0].args.hash,
            odbtools.utils.hashResult(__taskid, __raw).hash,
            'check resultHash',
        );
    });
    ...
    it('[2.4][TAG] Contribute - Error (duplicate)', async () => {
        ...
        (results = await odbtools.utils.signContribution(
            odbtools.utils.sealResult(__taskid, __raw, __worker.address),
            __enclave,
        )),
            (authorization = await odbtools.utils.signAuthorization(
                { worker: __worker.address, taskid: __taskid, enclave: __enclave.address },
                broker.wallet,
            ));
        // First ok
        await sendContribution(__taskid, __worker, results, authorization, __enclave.address);
        // Second error
        await expectRevert.unspecified(
            sendContribution(__taskid, __worker, results, authorization, __enclave.address),
        );
    });
