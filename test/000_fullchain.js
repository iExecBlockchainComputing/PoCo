            ...
            describe('checks', async () => {
                it('deal', async () => {
                    ...
                    assert.equal(
                        deal.app.pointer,
                        AppInstance.address,
                        'check deal (deal.app.pointer)',
                    );
                    assert.equal(
                        deal.app.owner,
                        appProvider.address,
                        'check deal (deal.app.owner)',
                    );
                    assert.equal(
                        Number(deal.app.price),
                        apporder.appprice,
                        'check deal (deal.app.price)',
                    );
                    assert.equal(
                        deal.app.pointer,
                        requestorder.app,
                    ...
                    assert.equal(
                        ...
                        'check deal (deal.dataset.pointer)',
                    );
                    assert.equal(
                        deal.dataset.owner,
                        datasetProvider.address,
                        'check deal (deal.dataset.owner)',
                    );
                    assert.equal(
                        Number(deal.dataset.price),
                        datasetorder.datasetprice,
                        'check deal (deal.dataset.price)',
                    );
                    assert.equal(
                        deal.dataset.pointer,
                    ...
                    assert.equal(
                        ...
                        WorkerpoolInstance.address,
                        'check deal (deal.workerpool.pointer)',
                    );
                    assert.equal(
                        deal.workerpool.owner,
                        scheduler.address,
                        'check deal (deal.workerpool.owner)',
                    );
                    assert.equal(
                        Number(deal.workerpool.price),
                        workerpoolorder.workerpoolprice,
                        'check deal (deal.workerpool.price)',
                    );
                    ...
                    assert.equal(
                        Number(deal.schedulerRewardRatio),
                        5,
        ...
        describe('[2] initialization', async () => {
            ...
            describe('checks', async () => {
                it('task', async () => {
                    ...
                    assert.equal(
                        Number(task.timeref),
                        (await IexecInstance.viewCategory(requestorder.category)).workClockTimeRef,
                    );
            ...
            describe('worker 1', async () => {
                ...
                describe('checks', async () => {
                    it('contribution', async () => {
                        ...
                        // for (w of workers)
                        {
                            contribution = await IexecInstance.viewContribution(
                                taskid,
                                w.agent.address,
                            );
                            assert.equal(
                ...
                it('task', async () => {
                    ...
                    assert.equal(Number(task.winnerCounter), 0);
                    assert.deepEqual(
                        task.contributors.map((a) => a),
                        [workers[0].agent.address],
                    );
                });
