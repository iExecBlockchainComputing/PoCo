...
contract('IexecHub', async (accounts) => {
	...
	it("[Genesis] Check balances", async () => {
		...
		IexecClerkInstance.viewAccountLegacy(poolScheduler).then(balance => assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 0, 0 ], "check balance"));
		IexecClerkInstance.viewAccountLegacy(poolWorker1  ).then(balance => assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 0, 0 ], "check balance"));
		IexecClerkInstance.viewAccountLegacy(poolWorker2  ).then(balance => assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 0, 0 ], "check balance"));
		IexecClerkInstance.viewAccountLegacy(poolWorker3  ).then(balance => assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 0, 0 ], "check balance"));
		IexecClerkInstance.viewAccountLegacy(poolWorker4  ).then(balance => assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 0, 0 ], "check balance"));
		IexecClerkInstance.viewAccountLegacy(user         ).then(balance => assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 0, 0 ], "check balance"));
	});
