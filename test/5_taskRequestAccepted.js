...
contract('IexecHub', function(accounts) {
  ...
  it("scheduleProvider accept Pending taskRequest", async function() {
    ...
    assert.isBelow(txMined.receipt.gasUsed, amountGazProvided, "should not use all gas");
    events = await Extensions.getEventsPromise(aWorkerPoolInstance.TaskAccepted({}));
    assert.strictEqual(events[0].args.taskID, taskID, "taskID check");
    events = await Extensions.getEventsPromise(aIexecHubInstance.TaskAccepted({}));
    assert.strictEqual(events[0].args.taskID, taskID, "taskID check");
    assert.strictEqual(events[0].args.workerPool, workerPoolAddress, "workerPool check");
    m_statusCall = await aTaskRequestInstance.m_status.call();
