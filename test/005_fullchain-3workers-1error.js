...
contract('IexecHub', async (accounts) => {
	...
	it("[Finalized] Check balances", async () => {
		...
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker1  )).map(x => x.toNumber()), [ 1005, 10 ], "check balance"); // +15
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker2  )).map(x => x.toNumber()), [ 1005, 10 ], "check balance"); // +15
