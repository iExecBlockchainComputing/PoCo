...
contract('IexecHub', async (accounts) => {
	...
	before("configure", async () => {
		...
		trusttarget = 10;
		workers = [
			{ address: worker1, enclave: constants.NULL.ADDRESS, raw: "iExec the wanderer" },
			{ address: worker2, enclave: constants.NULL.ADDRESS, raw: "je ne veux pas travailler" },
	...
	it("[Finalized] Check balances", async () => {
		...
		balance = await IexecClerkInstance.viewAccount(worker1        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1000, 10 ], "check balance"); // +10
		...
		balance = await IexecClerkInstance.viewAccount(worker3        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [ 1010, 10 ], "check balance"); // +20
		balance = await IexecClerkInstance.viewAccount(worker4        ); assert.deepEqual([ Number(balance.stake), Number(balance.locked) ], [  990, 10 ], "check balance");
