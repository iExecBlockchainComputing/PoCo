...
contract('IexecHub', async (accounts) => {
	...
	it("[Finalized] Check balances", async () => {
		...
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker1  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 1005, 10 ], "check balance");
		balance = await IexecClerkInstance.viewAccountLegacy(poolWorker2  ); assert.deepEqual([ balance.stake.toNumber(), balance.locked.toNumber() ], [ 1005, 10 ], "check balance");
