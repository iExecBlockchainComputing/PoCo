...
contract('IexecHub', async (accounts) => {
	...
	it("Check balances - After", async () => {
		...
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolScheduler)).map(x => x.toNumber()), [ 1005,  0 ], "check balance"); // +5
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker1  )).map(x => x.toNumber()), [  995, 10 ], "check balance"); // +5
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker2  )).map(x => x.toNumber()), [  995, 10 ], "check balance"); // +5
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker3  )).map(x => x.toNumber()), [  995, 10 ], "check balance"); // +5
		assert.deepEqual((await IexecClerkInstance.viewAccountLegacy(poolWorker4  )).map(x => x.toNumber()), [  995, 10 ], "check balance"); // +5
