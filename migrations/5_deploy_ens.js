...
var ReverseRegistrar = artifacts.require(
    '@ensdomains/ens-contracts/contracts/registry/ReverseRegistrar',
);
    ...
    if (chainid > 1000) {
        ...
        function namehash(domain) {
            return domain
                .split('.')
                .reverse()
                .reduce((hash, label) => compose(labelhash(label), hash), '0x0');
        }
        ...
        async function registerDomain(label, domain = '') {
            ...
            if ((await web3.eth.getCode(owner)) == '0x') {
                registrars[name] = await FIFSRegistrar.new(ens.address, nameHash, {
                    from: accounts[0],
                });
                if (!!name) {
                    await registrars[domain].register(labelHash, registrars[name].address, {
                        from: accounts[0],
                    });
                } else {
            ...
            console.log(
                `FIFSRegistrar for domain ${name} deployed at address: ${registrars[name].address}`,
            );
        ...
        switch (deploymentOptions.asset) {
            case 'Token':
                IexecInterfaceInstance = await IexecInterfaceToken.at(
                    (await ERC1538Proxy.deployed()).address,
                );
                break;
            case 'Native':
