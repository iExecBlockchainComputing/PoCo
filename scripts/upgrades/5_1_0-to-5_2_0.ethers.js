...
artifacts = {
    require: (name) => {
        try {
            return require(`${process.cwd()}/build/contracts/${name}.json`);
        } catch {}
        try {
            return require(`${process.cwd()}/node_modules/${name}.json`);
        } catch {}
    },
};
...
class FactoryDeployer {
    ...
    async deploy(artefact, options = {}) {
        ...
        const libraryAddresses = await Promise.all(
            LIBRARIES.filter(
                ({ contractName }) => artefact.bytecode.search(contractName) != -1,
...
(async () => {
    ...
    // Deployer
    const deployer = new FactoryDeployer(GenericFactory, {
        wallet,
        chainid,
    ...
    const proxy = new ethers.Contract(
        ERC1538Proxy.networks[chainid].address,
        ERC1538Update.abi,
        wallet,
    );
