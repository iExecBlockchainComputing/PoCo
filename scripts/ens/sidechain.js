...
(async () => {
    ...
    await deployer.deploy(ENSRegistry, {
        call: new ethers.utils.Interface(ENSRegistry.abi).encodeFunctionData('setOwner', [
    ...
    await new Promise((resolve) => {
        Promise.all(
            domains
                ...
                .map(async (entry) => ({
                    ...entry,
                    owner: entry.owner || wallet.address,
                    _owner: await ens.owner(ethers.utils.namehash(entry.name)),
                })),
        ).then((_) => {
            _.filter(({ _owner, owner }) => _owner != owner)
    ...
    await new Promise((resolve) => {
        Promise.all(
            domains
                ...
                .map(async (entry) => ({
                    ...entry,
                    resolver: entry.resolver || publicresolver.address,
                    _resolver: await ens.resolver(ethers.utils.namehash(entry.name)),
                })),
        ).then((_) => {
            _.filter(({ _resolver, resolver }) => _resolver != resolver)
                .reduce(async (promise, entry) => {
                    await Promise.resolve(promise);
                    console.log(`setResolver(${entry.name}) → ${entry.resolver}`);
                    await (
                        await ens.setResolver(ethers.utils.namehash(entry.name), entry.resolver)
                    ).wait();
                }, Promise.resolve())
                .then(resolve);
        });
        ...
        Promise.all(
                ...
                .map(async (entry) => {
                    try {
                        const resolverContract = new ethers.Contract(
                            await ens.resolver(ethers.utils.namehash(entry.name)),
                            PublicResolver.abi,
                            wallet,
                        );
                        const address = await resolverContract['addr(bytes32)'](
            ...
            _.filter(Boolean)
                .reduce(async (promise, entry) => {
                    ...
                    await (
                        await entry.resolverContract['setAddr(bytes32,address)'](
                            ethers.utils.namehash(entry.name),
                            entry.address,
                        )
                    ).wait();
    ...
    await new Promise((resolve) => {
        Promise.all(
            domains
                ...
                .map((entry) => ({
                    ...entry,
                    lookup: `${entry.address.toLowerCase().substr(2)}.addr.reverse`,
                }))
                .map(async (entry) => {
                    try {
                        const reverseResolver = new ethers.Contract(
                            await ens.resolver(ethers.utils.namehash(entry.lookup)),
                            PublicResolver.abi,
                            wallet,
                        );
                        const name = await reverseResolver.name(
                            ethers.utils.namehash(entry.lookup),
                        );
                ...
                .reduce(async (promise, entry) => {
                    ...
                    await (
                        await new ethers.Contract(
                            entry.address,
                            [ethers.utils.FunctionFragment.fromString('setName(address,string)')],
                            wallet,
                        ).setName(ens.address, entry.name)
                    ).wait();
                }, Promise.resolve())
                .then(resolve);
        });
