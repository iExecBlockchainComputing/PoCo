...
import {
    ...
    hashOrder,
    signOrder,
    signOrderWithSmartAccountSigner,
    getDealId,
    getTaskId,
...
async function createAndSignAppOrder({
    ...
    try {
        ...
        await signOrderWithSmartAccountSigner(domain, appOrder, smartAccountClient);
...
async function createAndSignRequestOrder({
    ...
    try {
        ...
        if (usePreSign) {
            ...
            await signOrderWithSmartAccountSigner(domain, requestOrder, smartAccountClient);
            requestOrderHash = await hashOrder(domain, requestOrder);
            console.log(`Request order signed with hash: ${requestOrderHash}`);
        }
